...
const displayStr = `A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString()} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length === 1 ? "game" : "games"}. Currently, ${unfundedGamesCount}
...
/*****************************
 * Adding a search option to the page
 * Skills used: event listeners, filter, includes, toLowerCase
 */

// grabbing search input element
...
searchInput.addEventListener("input", function(event) {
    ...
    // filter the GAMES_JSON array based on the search term
    const filteredGames = GAMES_JSON.filter(game =>
        game.name.toLowerCase().includes(searchTerm)
    );

    // remove existing games from the page
    deleteChildElements(gamesContainer);
